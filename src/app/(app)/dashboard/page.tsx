                      ...
                      <div className="grid gap-5 xl:grid-cols-2">
                        <Card className={cn(DASHBOARD_SHELL_CLASS, 'flex min-h-[250px] flex-col', isModern && 'border-slate-200/80 bg-white/95')}>
                          ...
                          <CardContent className="space-y-2 p-3">
                            <StatTile label="Total Flown" value={formatHours(fleetTotals.totalHours)} hint={activeDashboardPeriodLongLabel || 'Selected date range'} tone="info" />
                            <StatTile label="Average Per Aircraft" value={formatHours(fleetTotals.averageUtilisation)} hint="Across active fleet in period" tone="neutral" />
                            <StatTile label="Highest Aircraft" value={selectedAircraft ? formatHours(selectedAircraft.loggedHours) : formatHours(0)} hint={selectedAircraft ? (selectedAircraft.aircraft.tailNumber || selectedAircraft.aircraft.abbreviation || `${selectedAircraft.aircraft.make} ${selectedAircraft.aircraft.model}`.trim()) : 'No aircraft logged yet'} tone="positive" />
                            <StatTile label="Target Hours" value={targetHoursLabel} hint="Current fleet target benchmark" tone="neutral" />
                          </CardContent>
                        </Card>
                          ...
                          <CardContent className="p-0">
                            <div className="divide-y">
                              {fleetRows.length > 0 ? (
                                ...
                                <div className="px-4 py-10 text-center text-sm text-muted-foreground">
                                  No aircraft hour data available yet.
                                </div>
                              )}
                            </div>
                          </CardContent>
