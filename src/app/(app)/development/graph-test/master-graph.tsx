...
import type { GraphPoint, GraphTemplate } from './graph-specs';
...
function ensureClosedEnvelope(envelope: GraphTemplate['envelope']) {
...
export function MasterGraph({
  ...
  const canvasHeight = compact
    ? Math.min(640, Math.max(580, Math.round(measuredWidth * 0.66 + 240)))
    : cozy
  ...
  const canvas = {
    ...
    height: canvasHeight,
    margin: compact
      ? { top: 26, right: 18, bottom: 60, left: 44 }
      : cozy
