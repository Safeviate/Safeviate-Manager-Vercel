...
const TableBuilderPage = () => {
  ...
  useEffect(() => {
    if (activeTemplate) {
        setTableData(activeTemplate.tableData);
    } else {
        initializeCells(3, 4);
    }
  }, [activeTemplate, initializeCells]);
  ...
  const addColumn = (index: number) => {
    setTableData(prev => {
        ...
        return { ...prev, cols: newCols, cells: newCells.sort((a,b) => a.r - b.r || a.c - b.c), colWidths: newColWidths };
    });
  };
  ...
  const addRow = (index: number) => {
      setTableData(prev => {
          ...
          return { ...prev, rows: newRows, cells: newCells.sort((a,b) => a.r - b.r || a.c - b.c), rowHeights: newRowHeights };
      });
  };
  ...
  const deleteColumn = (index: number) => {
      setTableData(prev => {
          ...
          const newCols = prev.cols - 1;
          const newCells = prev.cells.filter(cell => cell.c !== index).map(cell => {
              if (cell.c > index) {
          ...
          return { ...prev, cols: newCols, cells: newCells, colWidths: newColWidths };
      });
  };
  ...
  const deleteRow = (index: number) => {
      setTableData(prev => {
          ...
          const newRows = prev.rows - 1;
          const newCells = prev.cells.filter(cell => cell.r !== index).map(cell => {
              if (cell.r > index) {
          ...
          return { ...prev, rows: newRows, cells: newCells, rowHeights: newRowHeights };
      });
  };
  ...
  const handleMerge = () => {
    ...
    const newCells = tableData.cells.map(cell => {
      ...
      if (cell.r >= minR && cell.r <= maxR && cell.c >= minC && cell.c <= maxC) {
        return { ...cell, rowSpan: 1, colSpan: 1, hidden: true };
      }
      return cell;
    });
            ...
            <div className='flex justify-between items-center'>
             ...
             <div className='flex gap-2'>
               <SaveAsNewTemplateDialog onSave={handleSaveTemplate}>
                 <Button>
                   <Save className="mr-2" />
                   {activeTemplate ? `Save "${activeTemplate.name}"` : 'Save as New Template'}
                 </Button>
               </SaveAsNewTemplateDialog>
                {activeTemplate && (
                    <Button variant="outline" onClick={() => setActiveTemplate(null)}>New Blank Table</Button>
                )}
            </div>
        </CardContent>
      </Card>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {(savedTemplates || []).map(template => (
                    <Card key={template.id} className="flex items-center justify-between p-4">
                       ...
                       <Button variant="outline" size="sm" onClick={() => setActiveTemplate(template)}>
                           Load
                       </Button>
                    </Card>
                ))}
