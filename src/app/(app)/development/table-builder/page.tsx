...
export default function TableBuilderPage() {
    ...
    const handleMerge = () => {
        ...
        const boundingBoxWidth = maxC - minC + 1;
        const boundingBoxHeight = maxR - minR + 1;
        ...
        for (const key of selectedCells) {
            ...
            const cell = newCells.find(cell => cell.r === r && cell.c === c);
            if (cell) {
                selectedArea += cell.rowSpan * cell.colSpan;
            }
        }
    
        if (selectedArea !== boundingBoxArea) {
             toast({
                variant: "destructive",
        ...
        masterCell.rowSpan = boundingBoxHeight;
        masterCell.colSpan = boundingBoxWidth;
        ...
        newCells.forEach(cell => {
            if (cell.r >= minR && cell.r <= maxR && cell.c >= minC && cell.c <= maxC) {
                if (cell !== masterCell) {
                    cell.hidden = true;
                    // Reset spans of hidden cells to avoid future conflicts
                    cell.rowSpan = 1;
                    cell.colSpan = 1;
                }
            }
