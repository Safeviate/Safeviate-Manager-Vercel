...
export function AircraftForm({ tenantId, existingAircraft, isOpen, setIsOpen, trigger }: AircraftFormProps) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      if (existingAircraft) {
        ...
        addDocumentNonBlocking(colRef, { ...values, documents: [], components: [], maintenanceLogs: [] });
        toast({ title: 'Aircraft Registered' });
      }
      setIsOpen(false);
    } catch (error: any) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{existingAircraft ? 'Edit Aircraft' : 'Register New Aircraft'}</DialogTitle>
          <DialogDescription>Update the registration and meter readings for this asset.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <FormField control={form.control} name="model" render={({ field }) => (<FormItem><FormLabel>Model</FormLabel><FormControl><Input placeholder="PA-28-181" {...field} /></FormControl><FormMessage /></FormItem>)} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="make" render={({ field }) => (<FormItem><FormLabel>Make</FormLabel><FormControl><Input placeholder="Piper" {...field} /></FormControl><FormMessage /></FormItem>)} />
