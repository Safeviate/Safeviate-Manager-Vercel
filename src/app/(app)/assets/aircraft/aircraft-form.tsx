...
const aircraftSchema = z.object({
  ...
  frameHours: z.number({ coerce: true }).default(0),
  engineHours: z.number({ coerce: true }).default(0),
});
...
export function AircraftForm({ tenantId, existingAircraft, trigger }: AircraftFormProps) {
  ...
  const form = useForm<AircraftFormValues>({
    ...
    defaultValues: existingAircraft ? {
      ...
      type: existingAircraft.type as any || 'Single-Engine',
  ...
  const onSubmit = async (values: AircraftFormValues) => {
    ...
    try {
      if (existingAircraft) {
        ...
        const aircraftCol = collection(firestore, `tenants/${tenantId}/aircrafts`);
        addDocumentNonBlocking(aircraftCol, values);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>{trigger || <Button><Plus className="mr-2 h-4 w-4" /> Add Aircraft</Button>}</DialogTrigger>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{existingAircraft ? 'Edit Aircraft' : 'Add New Aircraft'}</DialogTitle>
          <DialogDescription>Manage airframe details and meter base-readings.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <FormField control={form.control} name="tailNumber" render={({ field }) => (<FormItem><FormLabel>Tail Number</FormLabel><FormControl><Input placeholder="e.g. ZS-KXY" {...field} /></FormControl><FormMessage /></FormItem>)} />
              ...
              <FormField control={form.control} name="model" render={({ field }) => (<FormItem><FormLabel>Model</FormLabel><FormControl><Input placeholder="e.g. 172P" {...field} /></FormControl><FormMessage /></FormItem>)} />
