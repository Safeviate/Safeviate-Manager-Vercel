...
const getDefaultFormValues = (aircraft?: Aircraft | null): AircraftFormValues => ({
    make: aircraft?.make || '',
...
export function AircraftForm({ tenantId, existingAircraft, onFormSubmit, trigger }: AircraftFormProps) {
  const firestore = useFirestore();
  const { toast } = useToast();
  const [isOpen, setIsOpen] = useState(!!trigger ? false : !existingAircraft);

  const form = useForm<AircraftFormValues>({
    resolver: zodResolver(formSchema),
    defaultValues: getDefaultFormValues(existingAircraft),
  });
  ...
  const handleOpenChange = (open: boolean) => {
    setIsOpen(open);
    if (!open) {
        form.reset(getDefaultFormValues(existingAircraft));
        if (onFormSubmit) onFormSubmit(); // To signal closing in parent
    }
  }
    ...
    <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <FormField control={form.control} name="abbreviation" render={({ field }) => ( <FormItem><FormLabel>Abbreviation</FormLabel><FormControl><Input placeholder="Max 5 chars" {...field} maxLength={5} /></FormControl><FormMessage /></FormItem> )} />
             ...
             <DialogFooter className="pt-4">
                <DialogClose asChild>
                    <Button type="button" variant="outline" onClick={() => { if(onFormSubmit) onFormSubmit() }}>Cancel</Button>
                </DialogClose>
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[700px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {existingAircraft ? `Update details for ${existingAircraft.tailNumber}.` : 'Fill in the details for the new aircraft.'}
          </DialogDescription>
        </DialogHeader>
        {formContent}
      </DialogContent>
