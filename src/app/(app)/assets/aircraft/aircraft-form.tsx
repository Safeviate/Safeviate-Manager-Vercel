...
export function AircraftForm({ existingAircraft, tenantId, onCancel, trigger }: AircraftFormProps) {
  ...
  const onSubmit = async (data: AircraftFormValues) => {
    ...
    try {
      ...
      onCancel(); // Close dialog or navigate away
      form.reset();
    } catch (error) {
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <ScrollArea className="h-[60vh]">
          <CardContent className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormField control={form.control} name="abbreviation" render={({ field }) => ( <FormItem><FormLabel>Abbreviation (5-char)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem> )} />
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => append({ id: uuidv4(), name: '', partNumber: '', manufacturer: '' })}>
                  ...
                  <Card key={field.id} className="bg-muted/30">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                       ...
                       <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <FormField control={form.control} name={`components.${index}.maxHours`} render={({ field }) => ( <FormItem><FormLabel>Max Hours (Lifespan)</FormLabel><FormControl><Input type="number" {...field} /></FormControl><FormMessage /></FormItem> )}/>
        ...
        <CardFooter className="border-t pt-6 flex justify-end gap-2">
          ...
          <Button type="submit" disabled={isSubmitting}>
            {isSubmitting ? 'Submitting...' : (existingAircraft ? 'Save Changes' : 'Add Aircraft')}
          </Button>
        </CardFooter>
      </form>
    </FormProvider>
