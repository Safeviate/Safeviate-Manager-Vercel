...
export function AircraftForm({ tenantId, existingAircraft, trigger }: AircraftFormProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      {trigger ? <div onClick={() => setIsOpen(true)}>{trigger}</div> : <Button onClick={() => setIsOpen(true)}>Add Aircraft</Button>}
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{existingAircraft ? 'Edit' : 'Add'} Aircraft</DialogTitle>
          <DialogDescription>Update the basic profile and meter readings for this aircraft.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
