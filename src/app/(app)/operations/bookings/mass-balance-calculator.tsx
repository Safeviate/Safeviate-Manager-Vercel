...
export const MassBalanceCalculator = ({ aircraft, initialData, onSave }: MassBalanceCalculatorProps) => {
  ...
  const getInitialStations = () => {
    // If the aircraft has a stations array defined, use it. Otherwise, create a default.
    if (aircraft.stations && aircraft.stations.length > 0) {
      return aircraft.stations.map(s => ({
        ...s,
        weight: 0, // Reset weights for a new calculation
        gallons: s.type === 'fuel' ? 0 : undefined,
      }));
    }
    // Default fallback
    return [
  ...
  useEffect(() => {
    const initialStns = getInitialStations();
    if (initialData) {
        setStations(prevStations => {
            return initialStns.map(station => {
  ...
  const handleSaveToBooking = () => {
    ...
    stations.forEach(station => {
        ...
        saveData[stationKey] = {
            ...
            moment: (parseFloat(String(station.weight)) || 0) * (parseFloat(String(station.arm)) || 0)
        }
    });
    saveData.basicEmpty = {
  ...
  const cgEnvelopeForChart = (aircraft.cgEnvelope || []).map(p => ({ x: p.cg, y: p.weight }));

  const allX = [...cgEnvelopeForChart.map(p => p.x), results.cg].filter(n => !isNaN(n) && isFinite(n));
  const allY = [...cgEnvelopeForChart.map(p => p.y), results.weight].filter(n => !isNaN(n) && isFinite(n));
  ...
  const xDomain: [number, number] = [ allX.length > 0 ? Math.min(...allX) - paddingX : 70, allX.length > 0 ? Math.max(...allX) + paddingX : 100 ];
  const yDomain: [number, number] = [ allY.length > 0 ? Math.min(...allY) - paddingY : 1000, allY.length > 0 ? Math.max(...allY) + paddingY : 3000 ];
      ...
      <div className="bg-card border border-border rounded-xl p-4 shadow-md relative min-h-[400px] flex flex-col justify-center items-center overflow-hidden">
          <ResponsiveContainer width="100%" height={350}>
              <ScatterChart margin={{ top: 20, right: 30, bottom: 20, left: 20 }}>
                  ...
                  {cgEnvelopeForChart.length > 0 && <Scatter name="Envelope" data={cgEnvelopeForChart} fill="transparent" line={{ stroke: 'hsl(var(--primary))', strokeWidth: 2 }} shape={() => null} isAnimationActive={false} />}
