      ...
      <Card className="min-h-[400px] flex h-full flex-1 flex-col overflow-hidden shadow-none border">
        ...
        <CardContent className="flex-1 min-h-0 p-0 bg-muted/5 overflow-hidden">
          <div className={cn('flex h-full min-h-0 flex-col overflow-y-auto', isMobile ? 'touch-pan-y' : '')}>
            {renderCapBoard(filteredAuditCaps)}
            ...
            {!hasVisibleContent ? (
              <div className="p-4">
                <div className="rounded-lg border border-dashed border-card-border bg-background/70 px-4 py-10 text-center">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] text-muted-foreground">No Tasks In This Company</p>
                  <p className="mt-2 text-sm text-muted-foreground">There are no corrective actions or supporting tasks for the selected company yet.</p>
                </div>
              </div>
            ) : null}
          </div>
